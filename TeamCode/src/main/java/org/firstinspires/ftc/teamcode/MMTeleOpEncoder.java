...
@TeleOp(name = "MM TeleOp Encoder")
//@Disabled

public class MMTeleOpEncoder extends OpMode {
    ...
    public void loop() {
        ...
        try {
            waitForTick(2000); // every 2ms, sample the encoder

            telemetry.addData("Front Left Drive", frontLeftDrive.getCurrentPosition());
            telemetry.addData("Front Right Drive", frontRightDrive.getCurrentPosition());
            telemetry.addData("Rear Left Drive", rearLeftDrive.getCurrentPosition());
            telemetry.addData("Rear Right Drive", rearRightDrive.getCurrentPosition());
    ...
    public void driveController(){
        if (!isRotatingToNearest90Degree && !isGettingOnPlatform){
            ...
            if (abs(leftStickX) < CargoBotConstants.CONTROLLER_DEAD_ZONE &&
                ...
                rearRightDrive.setPower(rightStickX);
