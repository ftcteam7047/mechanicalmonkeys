...
public class BitmapUtils {
    ...
    public static Bitmap resizeBitmap(Bitmap input, int inputSize){
        if(input.getWidth()==input.getHeight()){
            return Bitmap.createScaledBitmap(input,inputSize,inputSize,false);
        }
        int width=input.getWidth(),height=input.getHeight();
        Bitmap b=width<height? Bitmap.createScaledBitmap(input,inputSize,inputSize*height/width,false): Bitmap.createScaledBitmap(input,inputSize*width/height,inputSize,false);
        Bitmap output= ThumbnailUtils.extractThumbnail(b,inputSize,inputSize);
        return output;
    }
